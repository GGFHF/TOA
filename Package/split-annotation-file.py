...
def build_parser():
    ...
    parser.add_argument('--type', dest='type', help=f'Type of the first annotation file (mandatory): {xlib.get_type_code_list_text()}')
    parser.add_argument('--header', dest='header', help=f'There is a header record in the annotation file: {xlib.get_header_code_list_text()}; default: {xlib.Const.DEFAULT_HEADER}.')
    parser.add_argument('--rnum', dest='record_number_per_file', help=f'Record number per splitted file; default: {xlib.Const.DEFAULT_RNUM}.')
    ...
    while record != '':
        ...
        if output_record_counter > record_number_per_file:
            ...
            if annotation_file.endswith('.csv'):
                output_file = f'{annotation_file[:-4]}-{output_file_counter:02d}.csv'
            elif annotation_file.endswith('.csv.gz'):
                output_file = f'{annotation_file[:-7]}-{output_file_counter:02d}.csv.gz'
        ...
        while record != '' and key == key_old:
            ...
            xlib.Message.print('verbose', f'\rRead annotations: {input_record_counter} - Written annotations in the file {os.path.basename(output_file)}: {output_record_counter}.')
