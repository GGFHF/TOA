...
def form_list_results_logs():
    ...
    if OK:
        ...
        if result_dataset_id_list == []:
            print('*** WARNING: There is not any result dataset of the experiment/process {0}.'.format(experiment_id))
        else:
...
def form_view_result_log():
    ...
    if OK:
        ...
        if result_dataset_id == '':
            print('WARNING: The experiment/process {0} does not have result datasets.'.format(experiment_id))
            OK = False
    ...
    if OK:
        log_file = '{0}/{1}/{2}/{3}'.format(toa_config_dict['RESULT_DIR'], experiment_id, result_dataset_id, xlib.get_run_log_file())
